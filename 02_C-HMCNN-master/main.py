...
def main():
    ...
    parser.add_argument('--fractions', type=str,
                        ...
                        help='Comma-separated fractions of training data to use')
    parser.add_argument('--results_file', type=str, default=None,
                        help='Optional output CSV file path for incremental results')
    ...
    different_from_0 = torch.tensor(np.array((test.Y.sum(0)!=0), dtype = np.bool_), dtype=torch.bool)
    ...
    results_path = args.results_file if args.results_file else os.path.join(results_dir, dataset_name + '_incremental.csv')

    with open(results_path, 'a') as f:
        for frac in fractions:
            subset_size = int(len(train_dataset) * frac)
            subset_train_dataset = train_dataset[:subset_size]

            train_loader = torch.utils.data.DataLoader(dataset=subset_train_dataset, 
                                                    batch_size=batch_size, 
                                                    shuffle=True)
            ...
            model.to(device)
            print("Model on gpu for frac {}".format(frac), next(model.parameters()).is_cuda)
            ...
            for epoch in range(num_epochs):
                ...
                for i, (x, labels) in enumerate(train_loader):
                    ...
                    correct_train = (predicted == labels.bool()).sum()
